-- TOU-6: confirm_contact_booking should create (or attach to) the matching
-- Event/Backstage automatically, closing the gap where a confirmed booking
-- has no corresponding events row until someone recreates it by hand.
--
-- Adds the ongoing dedupe key for events (events.venue_booking_date_id,
-- analogous to the one-time-only migrated_booking_group_key used by the
-- 20260522172524 backfill) and an explicit bookings.event_id link, then
-- extends confirm_contact_booking to get-or-create the event and attach an
-- accepted event_artist_memberships row, synthesizing the events NOT NULL
-- columns the Inbox confirm flow never collects using the same placeholder
-- pattern the backfill migration already established.

alter table public.bookings
  add column event_id uuid references public.events(id) on delete set null;

alter table public.events
  add column venue_booking_date_id uuid
    references public.venue_booking_dates(id) on delete set null;

create unique index events_venue_booking_date_id_key
  on public.events(venue_booking_date_id)
