alter table public.bookings
  drop constraint if exists bookings_thread_id_key;

create index if not exists bookings_thread_id_idx
  on public.bookings(thread_id, updated_at desc);

create unique index if not exists bookings_active_pair_date_unique
  on public.bookings(band_id, venue_id, show_date)
  where status in ('confirmed', 'cancellation_requested');

drop function if exists public.request_booking_cancellation(uuid, text);
drop function if exists public.resolve_booking_cancellation(uuid, text, text);
drop function if exists public.cancel_confirmed_booking(uuid, text);
...
begin
  ...
  if not found then
    raise exception 'No active booking was found.';
  end if;
