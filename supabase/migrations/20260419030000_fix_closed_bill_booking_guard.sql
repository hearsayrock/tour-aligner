-- Fix: confirm_contact_booking allowed a booking on a date with is_closed_to_more_bands=true
-- when confirmed_count was 0. The guard incorrectly required count > 0, so a venue could
-- pre-close a date via the calendar editor and still accidentally confirm a booking on it.
