-- Rollback for 20260723225618_auto_create_event_on_booking_confirm.sql
--
-- Restores confirm_contact_booking to its prior definition (verbatim from
-- 20260419030000_fix_closed_bill_booking_guard.sql) before dropping the new
-- columns/index, so the function never references them mid-rollback.
...
drop index if exists public.events_venue_booking_date_id_key;

alter table public.events
  drop column if exists venue_booking_date_id;
