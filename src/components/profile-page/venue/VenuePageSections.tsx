...
export function createVenuePageSections({ venue, isOwner, isEditing, onEditContent, availabilityContent, privateChatContent, profileManagementContent }: {
  ...
  return [
    { sectionId: 'overview', content: <Card eyebrow="Overview" title="Room profile" action={<EditButton label="Edit facts" onClick={onEditContent ? () => onEditContent('facts') : undefined} />}><div className="grid gap-5 sm:grid-cols-2"><DetailRow icon={MapPin} label="Location" value={location} />{venue.capacity && <DetailRow icon={Users} label="Capacity" value={`${venue.capacity.toLocaleString()} cap`} />}{venue.age_requirement && <DetailRow icon={ShieldCheck} label="Age policy" value={AGE_LABELS[venue.age_requirement]} />}<DetailRow icon={Music2} label="Bill target" value={`${venue.default_bill_cap} act${venue.default_bill_cap === 1 ? '' : 's'} by default`} /></div></Card> },
